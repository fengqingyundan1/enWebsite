<template>
    <div>
        <el-form label-position="top" ref="" :model="climbForm" label-width="200px">
            ...
            <el-form-item v-show="!isMobile" style="text-align: left">
                <el-row :gutter="20" class="w_fpcus">
                    <el-col :span="6" v-for="(item,index) in climbForm.otherVideos" >
                        ...
                        <el-button type="primary" v-show="index<climbForm.otherVideos.length-1" name="otherVideo"  @click.native="toRight(index,$event)"><i class="el-icon-arrow-right" ></i> </el-button>
            ...
            <el-form-item style="text-align: left">
                <el-row :gutter="20" class="w_fpcus">
                    <el-col :span="6" v-for="(item,index) in climbForm.focusLists" name="focusList" >
                        ...
                        <el-button type="primary" v-show="index<climbForm.focusLists.length-1" name="focusList"  @click.native="toRight(index,$event)"><i class="el-icon-arrow-right" ></i> </el-button>
            ...
            <el-form-item style="text-align: left" v-show="isMobile">
                <el-row :gutter="20"  class="w_fpcus">
                    <el-col :span="10" v-if="climbForm.video[0].srcUrl">
                        <video  controls="controls" :poster="climbForm.video[0].poster" >
                            <source :src="climbForm.video[0].srcUrl" >
                            Your browser does not support the video tag.
                        </video>
                    </el-col>
                    <el-col :span="10" v-if="climbForm.video[0].srcUrl">
                        <h3 class="title">{{climbForm.video[0].title}}</h3>
                        <p class="info">{{climbForm.video[0].summary}}</p>
                    </el-col>
                    <el-col :span="2" v-if="climbForm.video[0].srcUrl">
                        <!--/*toDel2*/-->
            ...
            <el-form-item style="text-align: left" v-show="isMobile">
                <el-row :gutter="20"  class="w_fpcus">
                    <el-col :span="8" v-for="(item,index) in climbForm.videoLists" >
                        ...
                        <el-button type="primary" v-show="index<climbForm.videoLists.length-1" name="videoList"  @click.native="toRight(index,$event)"><i class="el-icon-arrow-right" ></i> </el-button>
                        <!--del3-->
            ...
            <el-form-item style="text-align: left">
                <el-row :gutter="20" class="w_fpcus">
                    <el-col :span="6" v-for="(item,index) in climbForm.productLists" >
                        ...
                        <el-button type="primary" v-show="index<climbForm.productLists.length-1" name="productList"  @click.native="toRight(index,$event)"><i class="el-icon-arrow-right" ></i> </el-button>
                        <!--del4-->
    ...
    export default{
        ...
        methods:{
            getDataLists:function(){
                let that=this
                that.climbForm.bannerImgUrl=''
                const obj1={}
                obj1.terminal_type=that.climbForm.terminal_type
                obj1.lanauage_type=that.climbForm.lanauage_type
                obj1.menu_type='Climb'
                that.$http({
                    url:'/api/gethomeinfo',
                    method:'post',
                    data:obj1,
                    ...
                    if(res.data.bannerImgUrl){
                        that.climbForm.bannerImgUrl=res.data.bannerImgUrl
                    ...
                    if(res.data.video){
                        that.climbForm.video=res.data.video
                    }
                    that.climbForm=res.data
                })
            },
            getValue:function(value){
                let that=this
                that.climbForm.bannerImgUrl=value
            },
            ...
            getDatas:function(value){
                const that=this
                if(that.dialog===0){
                    that.climbForm.focusLists.push(value)
                }else if(that.dialog===1){
                    that.climbForm.video[0].title=value.title
                    that.climbForm.video[0].summary=value.info
                    that.climbForm.video[0].srcUrl=value.videoSrc
                    that.climbForm.video[0].poster=value.imageUrl
                }else if(that.dialog===2){
                    that.climbForm.videoLists.push({
                        poster: value.imageUrl,
                        src:value.videoSrc
                    })
                }else if(that.dialog===3){
                    that.climbForm.productLists.push(value)
                }else if(that.dialog===4){
                    that.climbForm.otherVideos.push(value)
                }
                that.hasTitle=true
            },
            ...
            delConfirm:function(value){
                ...
                if(that.delIt===true){
                    switch(that.state){
                        case 1:
                            that.climbForm.focusLists.splice(that.focusListIndex,1)
                            break;
                        case 2:
                            that.climbForm.video[0].summary=''
                            that.climbForm.video[0].title=''
                            that.climbForm.video[0].srcUrl=''
                            that.climbForm.video[0].poster=''
                            break;
                        case 3:
                            that.climbForm.videoLists.splice(that.videoListIndex,1)
                            break;
                        case 4:
                            that.climbForm.productLists.splice(that.productIndex,1)
                            break;
                        case 5:
                            that.climbForm.otherVideos.splice(that.otherVideoIndex,1)
                            break;
                    }
                }
            ...
            onSubmit:function(){
                ...
                that.$http({
                    ...
                    transformRequest:[function (data, headers) {
                        headers['Content-Type']='application/json'
                        return JSON.stringify(that.climbForm)
                    }]
                }).then(function(res){
//                    console.log(res)
                    if(res.data.code === 1){
            ...
            toLeft:function(index,e){
                ...
                if(name==='focusList'){
                    let temp=that.climbForm.focusLists[index]
                    that.$set(that.climbForm.focusLists, index , that.climbForm.focusLists[index-1]);
                    that.$set(that.climbForm.focusLists, index-1, temp)
                }else if(name==='videoList'){
                    let temp=that.climbForm.videoLists[index]
                    that.$set(that.climbForm.videoLists, index , that.climbForm.videoLists[index-1]);
                    that.$set(that.climbForm.videoLists, index-1, temp)
                }else if(name==='productList'){
                    let temp=that.climbForm.productLists[index]
                    that.$set(that.climbForm.productLists, index , that.climbForm.productLists[index-1]);
                    that.$set(that.climbForm.productLists, index-1, temp)
                }else if(name==='otherVideo'){
                    let temp=that.climbForm.otherVideos[index]
                    that.$set(that.climbForm.otherVideos, index , that.climbForm.otherVideos[index-1]);
                    that.$set(that.climbForm.otherVideos, index-1, temp)
                }
            },
...
//                console.log(plat)
                if(plat===0){
                    that.climbForm.terminal_type=1
                    that.climbForm.lanauage_type=2
                }else if(plat===1){
                    that.climbForm.terminal_type=2
                    that.climbForm.lanauage_type=2
                }else if(plat===2){
                    that.climbForm.terminal_type=1
                    that.climbForm.lanauage_type=1
                }else if(plat===3){
                    that.climbForm.terminal_type=2
                    that.climbForm.lanauage_type=1
                }
